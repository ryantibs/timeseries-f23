...
particulate matter, and 11 from temperature). However, make sure you do this in
...
half of the data set), you will have to additionally store the lasso solutions
along your time series CV pass. Then, look back at the solutions that correspond
to the MAE-optimal $\lambda$ value, and choose some way of summarizing which of
...
\]
Compute the matrix $K$ empirically for a problem of size $n=100$; inspect three
of its rows, at indices $i = 25, 50, 75$. For each $i$, plot the $i^{\text{th}}$ 
row as a curve over the underlying position $1,\dots,n$; that is, plot the x-y
pairs
\[
...
that the HP filter acts like a kernel smoother.
    ...
    Bonus: do a literature search to find theory on the *asymptotically 
equivalent kernel* for the HP filter. This should have a closed-form. Plot this
and comment on whether or not your empirical results adhere to what is known 
...
points for one fold. You can build off this code for your solution. (Note: you
will need to install the `glmgen` package from GitHub, which you can do using 
the code that has been commented out.)
...
tf = trendfilter(x = boston$Year[inds != 1], 
                 y = boston$Minutes[inds != 1], 
                 k = 1)

# Plot the predictions at the 25th lambda value
yhat = predict(tf, x.new = boston$Year[inds == 1])
