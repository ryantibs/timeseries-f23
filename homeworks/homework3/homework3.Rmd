...
7. The Hodrick-Prescott (HP) filter is defined by solving
\[
\min_{\theta \in \mathbb{R}^n} \, \sum_{i=1}^n (y_i - \theta_i)^2 + \lambda
\sum_{i=1}^{n-2} (\theta_i - 2\theta_{i+1} + \theta_{i+2})^2.
\]
Recall in lecture we saw it could be written its solution can be written as
\[
\hat\theta = \underbrace{(I + \lambda D^T D)^{-1}}_K \, y.
\]
In other words, defining $K$ as above, we have
\[
y_i = \sum_{j=1}^n K_{ij} y_j, \quad i = 1,\dots,n.
\]
...
equivalent kernel* for the HP filter. This should have a a closed-form. Plot
this and comment on whether or not your empirical results adhere to what is 
known about the asymptotics.
