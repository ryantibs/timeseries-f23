...
\item The Prophet model is a particular type of signal plus noise model, where
  we decompose the signal into three components, 
  \[
...
\item A popular time series forecaster based on LSTMs is called \emph{DeepAR},
  ...
  signal-to-noise ratio. (Another strength it is often cited as having is that
  it can learn rich, joint correlation structures between \emph{multiple} time
  series---think of spatiotemporal data, where we have one time series per
  location, and many locations---and it can and produce forecasts which respect
...
\begin{itemize}
\item So, you now have lagged regression, ARIMA, ETS, Theta, Prophet, and DeepAR
  in your forecasting toolkit. You have data in front of you, and you're unsure 
  which model(s) will work best. What do you do?

\item The answer we have given all semester: \emph{use time series CV}. That is,
  move the time index back to $t = t_0$, train each of them on the past, make
  forecasts, compare to the unseen target values, march the time index forward
  sequentially, and repeat. In doing so, we can compute our favorite accuracy
  metric (MAE, MASE, etc.) for each one, and choose the model that emerges as
  the most accurate, perhaps using Occam's razor to help decide between models
  that perform similarly

\item Here is an alternative that can often be advantageous: use the
  out-of-sample predictions from time series CV to inform how to best 
  \emph{combine} the models, rather than to select in between them    
...
\item At a high level, an ideal ensemble model would satisfy the following two
  properties:
  \begin{enumerate}
  \item \emph{do no harm}: the ensemble should not do much worse than model
    selection, i.e., not much worse than the best constituent model
  \item \emph{robustness}: the ensemble should have much more robust performance
    (fewer wild errors) than the typical constituent model 
  \end{enumerate}
...
\item Interestingly (though perhaps not surprisingly), these two are somewhat at 
  ...
  ensemble is flexible enough), but ``heavier'' training tends to hurt
  robustness 
\end{itemize}
...
\begin{itemize}
\item The simplest untrained method is just to take a straight average of all
  constituent model forecasts. Denote these by \smash{$\hat{x}^j_{t+h|t}$}, for
  $j = 1,\dots,p$. Then the simple average ensemble forecast is
  \[
...
\item Despite their simplicity, these methods can be very effective, especially
  ...
  of diversity in their errors), which can lead to very robust average or median
  ensembles
...
\item For state-level forecasts made over a 1.5 year period during the pandemic, 
  ...
  figure. We assign a standardized rank of 1 means to the most accurate forecast
  of all available, and 0 to the least accurate forecast. Then, for each
  forecast model, we average these standardized ranks over all forecast tasks,
  and a histogram of their standardized rank distribution is plotted
...
\item One of the simplest trained ensemble methods is to directly fit a weighted 
  ...
  etc.). For example, going by MSE, to form the ensemble at time $t$ (for a
  forecast of the target value at time $t+h$), we would first solve  
  \begin{alignat*}{2}
  &\min_{w \in \R^p} && \hspace{-6pt} \sum_{s=t_0+1}^t \bigg( x_s - \sum_{j=1}^p
  w_j \cdot \hat{x}^j_{s|s-h} \bigg)^2 \\   
...
\item After obtaining the solution \smash{$\hat{w}^t_j$}, $j = 1,\dots,p$, we
  ...
  \[
  \hat{x}^{\text{stack}}_{t+h|t} = \sum_{j=1}^p \hat{w}^t_j \cdot
  \hat{x}^j_{t+h|t} 
  \]
...
\item Instead of completely re-solving the optimization problem above, at each
  forecast time $t$, we could also incrementally update the learned weights over
  time. In fact, a method called (online) \emph{mirror descent} applied to the
  above optimization problem yields the following intuitive weight updates, at
  time $t$:   
  \begin{align*}
  \tilde{w}^t_j &= \hat{w}^{t-1}_j \exp\bigg\{ \eta \cdot \hat{x}^j_{t|t-h}
    \bigg( x_t -  \sum_{j=1}^p \hat{w}^{t-1}_j \cdot \hat{x}^j_{t|t-h} \bigg)
  ...
  forecaster's latest prediction \smash{$\hat{x}^j_{t|t-h}$} is correlated with
  the ensemble's latest residual \smash{$x_t -  \sum_{j=1}^p \hat{w}^{t-1}_j
    \cdot \hat{x}^j_{t|t-h}$}, then it is underrepresented in the ensemble, and
...
\item Stacking---especially more flexible second-level models that are
  ...
  (albeit in a batch prediction, rather than a time series context). The figure
  shows a specific deep neural network ensemble, called DQA (deep quantile
  aggregation) that is trained as a second-layer model on top of many
  constituent models, including some very flexible ones (such as deep neural
  networks themselves). Across 35 data sets, ordered by their signal-to-noise
  ratio along the x-axis, the WIS (recall this is a generalization of absolute
  error for quantile-based predictions) of all of the constituent models and
  other ensemble methods is compared to that of DQA. No method really
  outperforms DQA (nothing really below the red horizontal line at 1) and for
  problems with more signal, towards the right end of the x-axis, we can see
  that DQA improves massively on a lot of the other models 
...
\item How do we measure the accuracy of such prediction intervals? Let's fix a 
  horizon $h$ henceforth. Denote by \smash{$I^{1-\alpha}_ {t+h|t}$} denote the
  level $1-\alpha$ prediction interval made by our forecaster at time $t$, for
  the target variable $x_{t+h}$ at time $t+h$  

\item Recall, for a 90\% level, this is often (though not necessarily) of the
  form   
  \[
  I^{1-\alpha}_{ t+h|t} = 
...
\item Given these intervals, we can measure their empirical \emph{coverage}, in
  a time series CV sense: 
  \[
...
n important topic in probabilistic forecasting which we have not really
  covered thus far is \emph{calibration}.
\end{itemize}
