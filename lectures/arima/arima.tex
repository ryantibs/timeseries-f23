...
\item Figure \ref{fig:auto-cor} shows example auto-correlation functions for
  simple MA and AR models

\begin{figure}[htb]
\centering
\includegraphics[width=0.85\textwidth]{fig/ma-1.pdf}
\caption{Sample auto-correlation functions for ...}
\label{fig:auto-cor}
\end{figure}
\end{itemize}
...
\item Such a modification will be given to us by the \emph{partial
  ...
  population sense, so that precisely \smash{$\hat{X} = \Cov(Z)^{-1} \E(ZX)$}
  and \smash{$\hat{Y} = \Cov(Z)^{-1} \E(ZY)$}  
...
\item Figure \ref{fig:partial-auto-cor} ....

\begin{figure}[htb]
\centering
\includegraphics[width=0.85\textwidth]{fig/ma-1.pdf}
\caption{Sample partial auto-covariance functions for ...}
\label{fig:partial-auto-cor}
\end{figure}
...
\item The table below summarizes the behavior of the auto-correlation function
  ...
  Section 3.3 of SS if you are curious. Same with the behavior of causal,
  invertible ARMA($p,q$) 
\end{itemize}
