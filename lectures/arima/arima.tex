...
\caption{Top row: number of US jobs (in millions) in leisure/hospitality. Middle
  row: seasonal (yearly) differences. Bottom row: differences of seasonal
  differences.} 
\label{fig:leisure-exploratory}
\end{figure}

\item ACF and PACF plots are displayed in the top row of Figure
  \ref{fig:leisure-forecasts}. To be clear, these are applied to the differences
  of yearly differences ($\nabla \nabla_s x_t$, $t = 1,2,3,\dots$, as plotted in
  the bottom row of Figure \ref{fig:leisure-forecasts}). The ACF shows two
...
\item These are shown in the bottom row of \ref{fig:leisure-forecasts}. They
...
\item The answer is almost certainly ``no'', in any application! You should be 
  ...
  limited for long-term forecasts (in general, long-term forecasts are very hard
  without precise domain-specific knowledge of what the long-range dynamics look
  like ... and even then, most domains to not offer much certainty there!)  
...
\begin{itemize}
\item Finally, we arrive at our bread-and-butter tools for evaluating ARIMA
...
\item They apply exactly as described in the regression lecture: to recap, in
  ...
  would choose a smaller $t_0$ for time series CV than for split-sample) to
  ensure the initial model is decent, and then iteratively retrain at each $t >
  t_0$ in order to make forecasts. For $h$-step ahead forecasts:   
  \[
...
\item Finally, as a general rule-of-thumb, we recall Occam's razor: if a number
  of models all have reasonably similar split-sample or cross-validated
  prediction error, then we would generally prefer to use the simplest among 
  them 
\end{itemize}
...
\item There are various ways to compute prediction intervals. One method is to
  ...
  this precisely, but we will avoid going into details because this approach is
  ...
  case, having computed such an estimate, we would model the $h$-step ahead
...
\item Another method is adapt the method described previously to iteratively
  ...
  with replacement) from the empirical distribution of ARIMA residuals. Given a
...
\item Fortunately, we will learn that there are some relatively simple methods
  that will endow prediction intervals with long-run coverage. We'll talk about
  this near the end of the course when we discuss calibration 
\end{itemize}
...
\item (Note: be careful not to confuse an ARIMAX model with a regression with 
  ...
  exogenous predictors and and errors with an MA dynamic, but the distinguishing 
  factor is the AR dynamic: in an ARIMAX model, the ``response'', which is $x_t$
  in our notation here, obeys an AR dynamic; in a regression with correlated
  errors, the errors obey an AR dynamic) 
\end{itemize}
