...
\subsection{Cautionary tale for CV}
...
\item The 1-step ahead prediction from this model is therefore  
  \begin{align*}
  \hat{x}_{t|t-1} &= \sum_{j=1}^\infty (1-\lambda) \lambda^{j-1} x_{t-j} \\
  &= (1-\lambda) x_{t-1} + \sum_{j=2}^\infty (1-\lambda) \lambda^{j-1} x_{t-j}
    \\
  &= (1-\lambda) x_{t-1} + \sum_{j=1}^\infty (1-\lambda) \lambda^j x_{t-1-j} \\
  &= (1-\lambda) x_{t-1} + \sum_{j=1}^\infty (1-\lambda) \lambda^{j-1} x_{t-1-j}
    \\
  &= (1-\lambda) x_{t-1} + \lambda \hat{x}_{t-1|t-2}
  \end{align*}

\item In other words, the 1-step ahead prediction is a weighted combination of
  the latest observation $x_{t-1}$ and the previous prediction
  \smash{$\hat{x}_{t-1|t-2}$}
