...
\item Before we describe these, we recall the following terminology: for a
  polynomial \smash{$f(z) = \sum_{j=0}^k a_j z^j$}, we say that $z$ is a
  \emph{root} of $f$ provided $f(z) = 0$
...
\item We won't cover the proofs of these properties or go into further details
  about them or their importance. You can see Appendix B.2 of SS for proofs,
  or read more in Chapter 3.1 of SS about these properties, and read other parts
  of the SS book for their consequences (and significance). At a high-level,
  they are worth knowing because they are considered foundational results in
  ARMA models (just like it is worth knowing foundations for regression models,
  etc.)  
