...
\item We won't cover estimation at all, but we'll just note that a simple
  approach is as follows (which dates back to Durbin in 1960). Start with some 
  estimates \smash{$\hat{w}_t$} the noise variates $w_t$. Then use these as
  covariates, i.e., regress $x_t$ on
...
\item At least, this topic seems to be more controversial ... some authors like
  ...
  \verb|ARIMA()| in the \verb|fable| package. But other authors like Stoffer 
  believe that this doesn't work,\footnote{See
...
\item But to HA's credit, their Chapter 9.8 does recommend more of a
  human-in-the-loop procedure, involving several diagnostic steps
...
\item The gist of the non-automated part (which is fairly standard) is as
  ...
  \item Fit an ARMA model, inspect the residuals: they should look like white 
    noise 
  \end{enumerate}
  Step 3 is at the heart of ARIMA, which we'll cover soon. Steps 4-5 are the
  part that get automated by Hyndman-Khandakar: instead of just one ARMA model, 
  it fits a number of different ARMA models (using a stepwise procedure) and
  then uses an information criterion (like AICc) to select a final one
