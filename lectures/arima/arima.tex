...
\item For an AR($p$) model \eqref{eq:ar-p}, assumed causal (i.e., all roots of
  $\phi(z)$ are outside of the unit circle), we can take the process at $t+h$,
  where $h \geq 0$ (without a loss of generality, because $\gamma$ is symmetric 
  around zero), then multiply both sides by $x_t$, and take an expectation to
  get      
  \[
...
\item Now what about an ARMA($p,q$) model \eqref{eq:arma-pq}, assumed
  causal? Similarly, we take the process at time $t+h$, $h \geq 0$, multiply by
  $x_t$, and take an expectation to get  
  \[
...
\item This is again a difference equation of order $p$ that determines $\gamma$,
  ...
  \eqref{eq:ar-p-auto-cov}, and the qualitative behavior will still be the same
  as in the AR($p$) case  

\item Figure \ref{fig:auto-cov} shows example auto-correlation functions for
  simple MA and AR models

\begin{figure}[htb]
\centering
\includegraphics[width=0.85\textwidth]{fig/ma-1.pdf}
\caption{Sample auto-covariance functions for ...}
\label{fig:auto-cov}
\end{figure}
\end{itemize}
...
\item The auto-covariance function for an MA model provides a considerable
  ...
  seeing where the sample auto-correlations ``drop off'', we could roughly
  identify the order $q$ of the underlying MA process (see Figure
  \ref{fig:auto-cov} again)   
...
\item For an AR (or ARMA) model, this is not the case. As we saw from
  ...
  us little about the AR order of dependence $p$ (see also Figure
  \ref{fig:auto-cov}). Thus it is worth pursuing a type of \emph{modified}
...
\item Such a modification will be given to us by the \emph{partial
  ...
  &\text{$\hat{X}$ is the regression of $X$ on $Z$}, \quad \text{and} \\
  &\text{$\hat{Y}$ is the regression of $Y$ on $Z$}
  \end{align*}
  Here, and throughout this subsection, by ``regression'' we mean linear
  regression in the population sense, so that precisely \smash{$\hat{X} =
    \Cov(Z)^{-1} \E(ZX)$} and \smash{$\hat{Y} = \Cov(Z)^{-1} \E(ZY)$} 
...
\item We are now ready to define the partial auto-correlation function for a
  ...
  &\phi_x(h) = \Cor(x_t - \hat{x}_t, x_{t+h} - \hat{x}_{t+h}), \quad
    \text{where} \\ 
  &\text{$\hat{x}_t$ is the regression of $x_t$ on $x_{t+1},\dots,x_{t+h-1}$}, 
  \quad \text{and} \\ 
  &\text{$\hat{x}_{t+h}$ is the regression of $x_{t+h}$ on
    $x_{t+1},\dots,x_{t+h-1}$}  
  \end{align*}

\item To best see the effect of this definition we can go straight back to the
  causal AR($p$) model. When $h>p$, it can be shown that the population 
...
\item Figure \ref{fig:partial-auto-cov} ....

\begin{figure}[htb]
\centering
\includegraphics[width=0.85\textwidth]{fig/ma-1.pdf}
\caption{Partial auto-covariance functions for ...}
\label{fig:partial-auto-cov}
\end{figure}

\item The table below summarizes the behavior of the auto-correlation function
  (ACF) and partial auto-correlation function (PACF) for AR($p$) and MA($q$)
  models. By ``tails off'' we mean decays to zero as $h \to \infty$ without
  dropping to zero exactly; by ``cuts off'' we mean drops to zero at a finite
  lag $h$
  \begin{center}
  ...
  \end{tabular}
  \end{center}
  The fact that the partial auto-correlation function for an MA($q$) model
  ``tails off'' was not derived in these notes, but you can read more in Section
  3.3 of SS if you are curious. Same with the behavior of ARMA($p,q$)
\end{itemize}
