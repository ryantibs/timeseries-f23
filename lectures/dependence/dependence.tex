...
\item The \texttt{acf()} function in R estimates the auto-covariance
  (\texttt{type = ``covariance''}) or auto-correlation (\texttt{type =
    ``correlation''}) function according to the formulas given above. An example
  is given in Figure \ref{fig:speech-acf}, for the speech data that we saw in
  the last lecture, plotted again in Figure \ref{fig:speech-ts}. We can see
  strong patterns here in the sample auto-correlation, which makes sense,
  because the original time series appears to be a sequence of repeating short
  signals   
...
\item The \texttt{ccf()} function in R estimates the cross-covariance or
  ...
  \texttt{ccf()} was already given in Figure \ref{fig:covid-ccf} for the
  Covid-19 data 
\end{itemize}
