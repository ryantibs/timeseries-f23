...
\item A \emph{multiplicative seasonality} version of Holt-Winters is also
  possible: 
  \begin{equation}
  \label{eq:holt-winters-multiplicative}
  \begin{aligned}
  \hat{x}_{t+h | t} &= (\ell_t + b_t h) s_{t+h-mk} \\   
...
\item The equations in \eqref{eq:holt-winters-multiplicative} are motivated and
  interpreted just as in \eqref{eq:holt-winters}, except that the contribution
...
\begin{itemize}
\item We've talked about seasonal-trend (ST) decompositions of time series at
  various points in the past, and touched on multiple approaches for fitting
  them

\item Here we note that the Holt-Winters method, either in additive or
  multiplicative form, is not just a forecaster but also provides us with an ST
  decomposition of the given time series by extracting the fitted level
  $\ell_t$, trend $b_t$, and seasonal $s_t$ sequences

\item There is a bit of an unfortunate clash of nomenclature here: the
  fitted level component from Holt-Winters actually provides what we called the
  trend component previously in discussions of time series decomposition! And 
  the fitted trend component from Holt-Winters does not have a correspondence to
  anything we talked about previously. It reflects ``where the time series is
  heading''    

\item Figure \ref{fig:st_decomposition} shows an example on the holiday travel
...
\begin{figure}[p]
\centering
\includegraphics[width=0.95\textwidth]{fig/st-decomposition-1.pdf}
\caption{Seasonal-trend decomposition on the Australian holiday travel data
  (from HA) from the Holt-Winters method in additive and multiplication forms.}  
...
\item In fact, ETS includes more: we can also change the model to accomodate a  
  ...
  component, as was previously introduced). This looks will be made more precise
  when we talk about the state space representation, below
...
\item Thus we an ETS model is written ETS($x,y,z$), where 
  ...
  \item $x \in \{ \text{N}, \text{A}, \text{Ad} \}$
  \item $x \in \{ \text{N}, \text{A}, \text{M} \}$
  \end{itemize}
...
\item Note: HA do not recommend combining additive errors with multiplicative
  seasonality, saying that this can lead to numerical instability. Thus they say 
  that ETS(A,*,M) should generally be ditched in favor of ETS(M,*,M) 
\end{itemize}
