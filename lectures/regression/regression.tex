...
\item We will not cover these. These are classical tools for variable/model
  selection. You can read about them in SS and/or HA if you are
  interested. We'll take a more predictive angle, and simply rely on
  cross-validation (oriented towards time series)
\end{itemize}
...
\item Beyond looking at the auto-correlation of the residuals, there are a
  ...
  many sources, including the R help file for \verb|plot.lm()|, and both the SS
  and HA books. We will not be able to cover them, for the sake of time, though
  they are worth being generally aware of, so worth reading up on separately
  (and/or hopefully you will see them in more detail in another course)
\end{itemize}

\begin{figure}[htb]
\centering
\includegraphics[width=0.875\textwidth]{fig/cardio-5.pdf} 
\caption{Auto-correlation function of the residuals for the linear regression of
  cardiovascular mortality on particular levels.}
...
\item You have to be very careful to remember that these are predicated on the
  correctness of the model \eqref{eq:model_n} (with fixed $X$). \emph{It is this
    ...
    look causality.} If this model is wrong (likely!) then these translations
  break down
