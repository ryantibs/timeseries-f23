...
\item Now we turn to a general fact about stationary processes in time series,
  which is called the \emph{spectral representation} of the auto-covariance
  function
...
\item The important high-level perspective to remember here: \emph{the
    ...
    about a time series, but express it in different ways}. The auto-covariance 
...
\item Recall our most basic stationary process which is white noise: $x_t$, $t = 
  ...
  variance $\sigma^2$. In other words, 
  \[

  \]\gamma(0) = \sigma^2$ and $\gamma( Precisely,   
  \begin{gather*}
